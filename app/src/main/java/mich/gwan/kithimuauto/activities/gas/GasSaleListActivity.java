...
import mich.gwan.kithimuauto.adapters.gas.GasSaleRecyclerAdapter;
import mich.gwan.kithimuauto.adapters.pdf.PDFBolt;
import mich.gwan.kithimuauto.adapters.pdf.PDFGas;
import mich.gwan.kithimuauto.databinding.ActivityGassalesListBinding;
...
public class GasSaleListActivity extends AppCompatActivity implements PDFGas.OnDocumentClose {
    ...
    private String path(){
        ...
        {
            path = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS) + "/"  + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"))+"_gas.pdf";
        }
        else
        {
            path = Environment.getExternalStorageDirectory() + "/Sarensa/Kithimu_Auto/" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"))+"_gas.pdf";
        }
        return path;
    }
    ...
    public void generateEXCEL(){
        ...
        String filename = m.format(t) + "_kithimuauto_gas_sale_data.xls";
    ...
    private void getDataFromSQLite() {
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected void onPostExecute(Void aVoid) {
                super.onPostExecute(aVoid);
                recyclerAdapter.notifyDataSetChanged();
                textNetProfit.setText((String.valueOf(databaseHelper.getAllGasSaleProfit())));
                textNetTotal.setText(String.valueOf(databaseHelper.getAllGasSaleTotal()));
            }
        }.execute();
    }
